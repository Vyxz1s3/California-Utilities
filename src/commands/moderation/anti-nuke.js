...
export default {
  ...
  async execute(interaction, client) {
    ...
    await query(
      `INSERT INTO guild_settings (guild_id, anti_nuke) VALUES ($1, $2)
       ON CONFLICT (guild_id) DO UPDATE SET anti_nuke = $2`,
      [interaction.guild.id, state]
    );

    const embed = new EmbedBuilder()
      .setColor(state ? 0x2ecc71 : 0xe74c3c)
      .setTitle(`💣 Anti-Nuke ${state ? 'Enabled' : 'Disabled'}`)
      .setDescription(`Anti-nuke protection has been **${state ? 'enabled' : 'disabled'}**.`)
      .setTimestamp();
